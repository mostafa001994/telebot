<?php

session_start();

require_once __DIR__ . '/../config/database.php';



if (isset($_SESSION['admin'])) {

    // User already authenticated.
    
} else {

 

    $telegramId = $_GET['telegram_id'] ?? null;

    if (!$telegramId) {
        exit("Unauthorized");
    }



    $stmt = $pdo->prepare("
        SELECT id, telegram_id, first_name, username, is_admin
        FROM users
        WHERE telegram_id = ?
        AND is_admin = 1
        LIMIT 1
    ");

    $stmt->execute([
        $telegramId
    ]);

    $admin = $stmt->fetch(PDO::FETCH_ASSOC);



    if (!$admin) {
        exit("Access denied");
    }



    $_SESSION['admin'] = $admin['id'];

    $_SESSION['telegram_id'] = $admin['telegram_id'];

    $_SESSION['admin_name'] =
        $admin['first_name'] ?? '';

    header("Location: ./");
    exit;
}

?>


<!DOCTYPE html>
<html lang="fa" dir="rtl">

<head>

    <meta charset="UTF-8">

    <meta
        name="viewport"
        content="width=device-width, initial-scale=1.0"
    >

    <title>پنل مدیریت</title>

    <link
        rel="stylesheet"
        href="./assets/style.css"
    >

</head>




<svg style="display: none;">

    <symbol
        id="users"
        viewBox="0 0 24 24"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
    >

        <circle
            opacity="0.4"
            cx="15"
            cy="6"
            r="3"
            fill="#1C274C"
        ></circle>

        <ellipse
            opacity="0.4"
            cx="16"
            cy="17"
            rx="5"
            ry="3"
            fill="#1C274C"
        ></ellipse>

        <circle
            cx="9.00098"
            cy="6"
            r="4"
            fill="#1C274C"
        ></circle>

        <ellipse
            cx="9.00098"
            cy="17.001"
            rx="7"
            ry="4"
            fill="#1C274C"
        ></ellipse>

    </symbol>


    <symbol
        id="subs"
        viewBox="0 0 24 24"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
    >

        <path
            opacity="0.5"
            d="M12.0006 16C6.2407 16 5.22032 10.2595 5.03956 5.70647C4.98928 4.43998 4.96414 3.80673 5.43985 3.22083C5.91557 2.63494 6.48494 2.53887 7.62367 2.34674C8.74773 2.15709 10.2171 2 12.0006 2C13.7842 2 15.2536 2.15709 16.3776 2.34674C17.5163 2.53887 18.0857 2.63494 18.5614 3.22083C19.0371 3.80673 19.012 4.43998 18.9617 5.70647C18.781 10.2595 17.7606 16 12.0006 16Z"
            fill="#cbc9c8"
        ></path>

        <path
            d="M17.6404 12.422L20.4569 10.8572C21.2093 10.4392 21.5855 10.2302 21.7927 9.87809C21.9999 9.52598 21.9999 9.09561 21.9999 8.23487L21.9999 8.16234C22 7.11873 22 6.59692 21.7168 6.20408C21.4337 5.81124 20.9387 5.64623 19.9486 5.31621L19 5L18.9831 5.08464C18.9784 5.27391 18.9702 5.48006 18.9612 5.70645C18.8729 7.93085 18.5842 10.4387 17.6404 12.422Z"
            fill="#cbc9c8"
        ></path>

        <path
            d="M5.03907 5.70647C5.12739 7.93096 5.41612 10.4389 6.36008 12.4223L3.54305 10.8572C2.79063 10.4392 2.41442 10.2302 2.20723 9.87809C2.00004 9.52598 2.00003 9.09561 2 8.23487L2 8.16234C1.99997 7.11874 1.99996 6.59692 2.2831 6.20408C2.56624 5.81124 3.06126 5.64623 4.05132 5.31621L4.99994 5L5.01728 5.08671C5.02196 5.27541 5.03011 5.4809 5.03907 5.70647Z"
            fill="#cbc9c8"
        ></path>

        <path
            fill-rule="evenodd"
            clip-rule="evenodd"
            d="M5.25 22C5.25 21.5858 5.58579 21.25 6 21.25H18C18.4142 21.25 18.75 21.5858 18.75 22C18.75 22.4142 18.4142 22.75 18 22.75H6C5.58579 22.75 5.25 22.4142 5.25 22Z"
            fill="#cbc9c8"
        ></path>

        <path
            opacity="0.5"
            d="M15.4582 21.25H8.54297L8.83979 19.5002C8.93327 19.0327 9.34368 18.6963 9.82037 18.6963H14.1808C14.6574 18.6963 15.0679 19.0327 15.1613 19.5002C15.0679 19.0327 14.6574 18.6963 15.4582 21.25Z"
            fill="#cbc9c8"
        ></path>

        <path
            d="M12.0002 16.0002C11.7406 16.0002 11.4907 15.9885 11.25 15.9658V18.6963H12.75V15.9658C12.5094 15.9885 12.2596 16.0002 12.0002 16.0002Z"
            fill="#cbc9c8"
        ></path>

    </symbol>


    <symbol
        id="credit-cart"
        viewBox="0 0 24 24"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
    >

        <path
            opacity="0.5"
            d="M21.9995 12.8175L21.591 12.409C20.7123 11.5303 19.2877 11.5303 18.409 12.409L17.6076 13.2104C17.2878 12.3573 16.4648 11.75 15.5 11.75C14.2574 11.75 13.25 12.7574 13.25 14V15.7638C12.7601 15.8183 12.2847 16.0334 11.909 16.409C11.0303 17.2877 11.0303 19.591 12.318 20H10C6.22876 20 4.34315 20 3.17157 18.8284C2 17.6569 2 15.7712 2 12C2 11.5581 2.00188 10.392 2.00377 10H22C22.0019 10.392 22 11.5581 21.9995 12.8175Z"
            fill="#1d3891"
        ></path>

        <path
            d="M5.25 16C5.25 15.5858 5.58579 15.25 6 15.25H10C10.4142 15.25 10.75 15.5858 10.75 16C10.75 16.4142 10.4142 16.75 10 16.75H6C5.58579 16.75 5.25 16.4142 5.25 16Z"
            fill="#1d3891"
        ></path>

        <path
            d="M9.99484 4H14.0052C17.7861 4 19.6766 4 20.8512 5.11578C21.6969 5.91916 21.9337 7.07507 22 9V10H2V9C2.0663 7.07507 2.3031 5.91916 3.14881 5.11578C4.3234 4 6.21388 4 9.99484 4Z"
            fill="#1d3891"
        ></path>

        <path
            d="M19.4697 13.4697C19.7626 13.1768 20.2374 13.1768 20.5303 13.4697L22.5303 15.4697C22.8232 15.7626 22.8232 16.2374 22.5303 16.5303C22.2374 16.8232 21.7626 16.8232 21.4697 16.5303L20.75 15.8107V20C20.75 20.4142 20.4142 20.75 20 20.75C19.5858 20.75 19.25 20.4142 19.25 20V15.8107L18.5303 16.5303C18.2374 16.8232 17.7626 16.8232 17.4697 16.5303C17.1768 16.2374 17.1768 15.7626 17.4697 15.4697L19.4697 13.4697Z"
            fill="#1d3891"
        ></path>

    </symbol>

</svg>


<body>

    <button id="menuBtn">
        ☰
    </button>


    <div class="sidebar">

        <h2>
            مدیریت
        </h2>

        <ul>

            <li>
                <a
                    href="#"
                    data-page="dashboard"
                >
                    داشبورد
                </a>
            </li>


            <li>
                <a
                    href="#"
                    data-page="users"
                >
                    کاربران
                </a>
            </li>


            <li>
                <a
                    href="#"
                    data-page="subscription"
                >
                    اشتراک‌ها
                </a>
            </li>


            <li>
                <a
                    href="#"
                    data-page="plans"
                >
                    پلن‌ها
                </a>
            </li>


            <li>
                <a
                    href="#"
                    data-page="payments"
                >
                    پرداخت‌ها
                </a>
            </li>

        </ul>

    </div>


    <div
        class="content"
        id="content"
    >
        در حال بارگذاری...
    </div>


    <script src="./assets/app.js"></script>

</body>

</html>

